...
async def read_users(session: SessionDep, skip: int = 0, limit: int = 100) -> Any:
    """Retrieve users."""
    users, count = await crud.list_users(session=session, skip=skip, limit=limit)
    return UsersPublic(
...
async def update_user(user_id: UUID, session: SessionDep, user_in: UserUpdate) -> Any:
    ...
    updated = await crud.update_user(session=session, user=db_user, user_update=user_in)
    await session.commit()
    return UserPublic.model_validate(updated)
