...
from app.models.v2.schemas import AttachmentUpdate, EntryCreate, EntryUpdate, TagCreate
from tests.sql.helpers import make_logbook


def _create(session: Session, **kwargs) -> Entry:
...
def test_get_entry(session: Session) -> None:
    entry = _create(session)
...
def test_timezone_aware_timestamps(session: Session) -> None:
    """created_at and occurred_at must round-trip with timezone info."""
    now = datetime.now(UTC)
    entry = _create(session, occurred_at=now)
...
def test_update_entry_bumps_version(session: Session) -> None:
...
def test_create_entry_with_logbook_ids(session: Session) -> None:
    """Creating an entry with logbook_ids produces LogbookEntry junction rows."""
    lb1 = make_logbook(session).id
    lb2 = make_logbook(session).id
    entry = _create(session, logbook_ids=[lb1, lb2])
...
def test_create_entry_with_tag_ids(session: Session) -> None:
    """Creating an entry with tag_ids produces EntryTag junction rows."""
    t1 = crud.create_tag(session=session, tag_in=TagCreate(name=f"tag-{uuid.uuid4()}")).id
    t2 = crud.create_tag(session=session, tag_in=TagCreate(name=f"tag-{uuid.uuid4()}")).id
    t3 = crud.create_tag(session=session, tag_in=TagCreate(name=f"tag-{uuid.uuid4()}")).id
    entry = _create(session, tag_ids=[t1, t2, t3])
...
def test_create_attachment(session: Session) -> None:
...
def test_create_attachment_with_optional_fields(session: Session) -> None:
...
def test_create_jira_link(session: Session) -> None:
    entry = _create(session)
    link = ExternalLink(
        entry_id=entry.id,
        system="jira",
...
def test_create_slack_link(session: Session) -> None:
    entry = _create(session)
    link = ExternalLink(
        entry_id=entry.id,
        system="slack",
...
def test_multiple_external_links_per_entry(session: Session) -> None:
...
def test_external_link_meta_roundtrip(session: Session) -> None:
...
def test_attachment_timestamps(session: Session) -> None:
...
def test_update_attachment(session: Session) -> None:
