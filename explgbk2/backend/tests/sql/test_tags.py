...
from app.models.v2.db import EntryTag
from app.models.v2.schemas import TagCreate, TagUpdate
...
def _make_tag(session: Session, **kwargs):
...
def test_create_tag(session: Session) -> None:
    tag = _make_tag(session, description="A test tag", color="#ff0000")
...
def test_update_tag(session: Session) -> None:
    tag = _make_tag(session, description="Original", color=None)
...
def test_update_tag_duplicate_name_raises(session: Session) -> None:
    tag_a = _make_tag(session)
    tag_b = _make_tag(session)
    session.flush()
    with pytest.raises(IntegrityError):
        crud.update_tag(
            session=session,
...
def test_tag_fk_on_entry_tag_enforced(session: Session) -> None:
